...
- Change how evapotranspiration is limited by dryness.
	...
	auto TotalQuickFlow     = RegisterEquationCumulative(Model, "Quick flow summed over land types", QuickFlow, LandscapeUnits, LandUseProportions);
	...
	*/
	...
	EQUATION(Model, SoilWaterFlow,
		double activ = SCurveResponse(RESULT(SoilWaterVolume), PARAMETER(SoilFieldCapacity), 1.01*PARAMETER(SoilFieldCapacity), 0.0, 1.0);
		return (RESULT(SoilWaterVolume) - PARAMETER(SoilFieldCapacity)) * activ / PARAMETER(SoilWaterTimeConstant);
	)
...
#ifdef SIMPLYQ_GROUNDWATER
		double fromland = RESULT(TotalQuickFlow) + (1.0 - PARAMETER(BaseflowIndex)) * RESULT(TotalSoilWaterFlow) + RESULT(GroundwaterFlow);
#else
		double fromland = RESULT(TotalQuickFlow) + RESULT(TotalSoilWaterFlow);
#endif
