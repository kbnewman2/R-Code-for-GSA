...
Reg: barplots of SRC
RegTree/Random Forest: Variable importance plots for both; regression tree; To produce heatmap of tree for Qsim, set heattree.on <- TRUE
GPR:  Inverse range and SRC

Set verbose <- TRUE below to see more intermediate output, etc.
...
# Truncating outputs, and then Sobol evaluations modified below
cat("full ymat \n")
head(ymat)

ymat <- ymat[,1:3] # data used in Environmental & Modelling SW
...
```{r, 2. Data Load, eval=KGE.switch, echo=FALSE,warning=FALSE, message=FALSE}
...
```{r, 3. KGE Metric, eval=KGE.switch, echo=FALSE}
# using y.Qsim.mat
...
## GR6J Sobol Time to evaluate= 0.1 minutes for 1000 comboss
...
## ## GR6J GPR.analysis Time to evaluate= 4.1 minutes for 1000 combos #actually 500
