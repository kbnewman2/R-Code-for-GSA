...
```{r, 1. Initial, eval=TRUE, include=FALSE, warning=FALSE,messages=FALSE}
dir.create(tempdir())
t1 <- proc.time()[3]
...
```{r, 2. Data Load, eval=TRUE, echo=TRUE}
...
```{r, 4. num.comb, message=FALSE, eval=TRUE}
...
```{r, 5. Prepare InputModel object, eval=TRUE, echo=TRUE}
...
```{r, 7. Plot Outputs, eval=TRUE, echo=TRUE}
# Examine the outputs 
...
```{r, 8B. Param combinations, eval=TRUE}
...
### 9. Making multiple runs with different param combinations, and selecting output to use for tGSA
```{r, 9. Multiple Runs, eval=TRUE, echo=TRUE}
...
if(verbose) {
  ...
  # cat("output of Qsim for KGE \n")
  # print(y.Qsim.mat[1:3,1:5])
  # print(y.Qsim.mat[(num.comb-3):num.comb,1:5])
}
...
```{r, 10. Save GR6J outputs,eval=TRUE}
switch(choice,
       Morris = {
...
## GR6J Sobol time= 0.48 Minutes w/ 1000 param combos
